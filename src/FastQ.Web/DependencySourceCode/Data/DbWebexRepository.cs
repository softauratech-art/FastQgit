...
namespace FastQ.Data.Db
{
    public sealed class DbWebexRepository : IWebexRepository
    {
        ...
        public WebexApiSettings GetApiSettings(string env)
        {
            try
            {
                string sp_name = "fqowner.FQ_WEBEX_SERVICE.GET_API_SETTINGS";
                using var conn = DataAccess.Open();
                using (var cmd = DataAccess.CreateStoredProc(conn, sp_name))
                {
                    DataAccess.AddParam(cmd, "p_env", env, DbType.String);                    
                    DataAccess.AddOutRefCursor(cmd, "p_ref_cursor");
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (!reader.Read())
                        ...
                        WebexApiSettings apidata = new WebexApiSettings
                        {
                             Environment = reader["ENVIRONMENT"]?.ToString(),
                             Base_Url = reader["BASE_URL"]?.ToString(),
                             Refresh_Url = reader["REFRESH_URL"]?.ToString(),
                             Client_Id = reader["CLIENT_ID"]?.ToString(),
                             Client_Secret = reader["SECRET_ID"]?.ToString(),
        ...
        public void UpdateApiSettings(string env, string responsejson)
        {
            try
            {
                using var conn = DataAccess.Open();
                string sp_name = "fqowner.FQ_WEBEX_SERVICE.UPDATE_API_SETTINGS";

                using (var cmd = DataAccess.CreateStoredProc(conn, sp_name))
                {
                    DataAccess.AddParam(cmd, "p_env", env, DbType.String);
                    DataAccess.AddParam(cmd, "p_json", responsejson, DbType.String);
                    DataAccess.AddParam(cmd, "p_outmsg", null, DbType.String).Direction = ParameterDirection.Output;
                    cmd.Parameters["p_outmsg"].Size = 4000;
                    cmd.ExecuteNonQuery();
                    //Ignore error
