...
namespace FastQ.Data.Db
{
    public sealed class DbAppointmentRepository : IAppointmentRepository
    {
        ...
        public long? GetCustomerIdForSource(char srcType, long sourceId)
        {
            ...
            using (var conn = DataAccess.Open())
            using (var cmd = DataAccess.CreateCommand(conn, $"SELECT CUSTOMER_ID FROM {tableName} WHERE {idColumn} = :sourceId"))
            {
        ...
        public bool HasCustomerScheduleConflict(long customerId, DateTime scheduledFor, char? excludedSourceType = null, long? excludedSourceId = null)
        {
            if (customerId <= 0)
            {
                return false;
            ...
            var normalizedExcludedType = excludedSourceType.HasValue
                ? char.ToUpperInvariant(excludedSourceType.Value).ToString()
                : string.Empty;
            var normalizedExcludedId = excludedSourceId.GetValueOrDefault();
...
SELECT COUNT(1)
 ...
 WHERE entries.SCHEDULED_FOR >= :minuteStart
   AND entries.SCHEDULED_FOR < :minuteEnd
   AND NOT (entries.SRC_TYPE = :excludedSourceType AND entries.SRC_ID = :excludedSourceId)";

            using (var conn = DataAccess.Open())
            using (var cmd = DataAccess.CreateCommand(conn, sql))
            {
                DataAccess.AddParam(cmd, "customerId", customerId, DbType.Int64);
                DataAccess.AddParam(cmd, "walkinCustomerId", customerId, DbType.Int64);
                DataAccess.AddParam(cmd, "minuteStart", minuteStart, DbType.DateTime);
                DataAccess.AddParam(cmd, "minuteEnd", minuteEnd, DbType.DateTime);
                DataAccess.AddParam(cmd, "excludedSourceType", normalizedExcludedType, DbType.String);
                DataAccess.AddParam(cmd, "excludedSourceId", normalizedExcludedId, DbType.Int64);
